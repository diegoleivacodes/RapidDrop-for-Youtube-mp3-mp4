...
class DownloaderApp(ctk.CTk):
    ...
    def ejecutar_descarga(self):
        ...
        ydl_opts = {
            ...
            # NUEVO: Vinculamos el proceso de conversión de FFmpeg con la interfaz
            'postprocessor_hooks': [lambda d: self.actualizar_progreso_conversion(d)],
        }
    ...
    def actualizar_progreso_descarga(self, d):
        if d['status'] == 'downloading':
            ...
            # Cuando termine de descargar el archivo crudo, preparamos la barra
            self.lbl_estado.configure(text="📥 Descarga completa. Pasando al procesador...", text_color="orange")

        # --- Coloca la función AQUÍ, dentro de la clase DownloaderApp (con 4 espacios de sangría) ---
    def actualizar_progreso_conversion(self, d):
        # Este hook escucha exclusivamente las tareas de postprocesamiento de FFmpeg
        if d['status'] == 'started':
            # Hace que la barra azul se mueva de lado a lado continuamente (Modo Animación)
            self.barra_progreso.configure(mode="indeterminate")
            self.barra_progreso.start()
            
            # Detectamos qué acción está haciendo para personalizar el texto del abue
            if "ExtractAudio" in d.get('postprocessor', ''):
                self.lbl_estado.configure(text="🎹 Convertidor: Comprimiendo audio a MP3... (Esto toma un momento)", text_color="orange")
            else:
                self.lbl_estado.configure(text="🎬 Procesador: Fusionando audio y video en MP4...", text_color="orange")
                
        elif d['status'] == 'finished':
            # Cuando FFmpeg termina, frenamos la animación y fijamos la barra al 100%
